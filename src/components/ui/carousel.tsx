...
const CarouselDots = React.forwardRef<
  HTMLDivElement,
  React.HTMLAttributes<HTMLDivElement> & { currentActive?: (active: number) => void }
>((props, ref) => {
  ...
  React.useEffect(() => {
    props.currentActive?.(currentSlide);
  }, [currentSlide]);
    ...
    return (
      <div ref={ref} className={`flex justify-center ${props.className}`}>
        {Array.from({ length: numberOfSlides }, (_, i) => (
          <Button
            key={i}
            className={`mx-2 h-3 w-3 rounded-full p-0 ${
              i === currentSlide
                ? 'w-[60px] bg-red-accent hover:bg-red-400'
